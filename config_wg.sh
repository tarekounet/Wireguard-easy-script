...
configure_values() {
    # Fonction pour gérer l'annulation par Ctrl+C ou Échap
    cancel_config() {
        trap - SIGINT
    ...
    if [[ ! -f "$DOCKER_COMPOSE_FILE" ]]; then
        ...
        echo "Le fichier docker-compose.yml est introuvable. Création d'un nouveau fichier..."
...
EOF
    ...
    if [[ -z "$CURRENT_WG_HOST" ]]; then
        ...
        read -p $'\e[0;32mUtiliser cette adresse IP publique ? (O/n, Échap pour annuler) : \e[0m' USE_AUTO_WG_HOST
        ...
        if [[ -z "$USE_AUTO_WG_HOST" || "$USE_AUTO_WG_HOST" == "o" || "$USE_AUTO_WG_HOST" == "O" ]]; then
            ...
            while true; do
                read -p $'\e[0;32mEntrez le nom de domaine ou l\'adresse publique (Échap pour annuler) : \e[0m' NEW_WG_HOST
        ...
        read -p $'\e[0;32mVoulez-vous modifier l\'adresse publique ? (o/N, Échap pour annuler) : \e[0m' MODIFY_WG_HOST
    ...
    if [[ -z "$CURRENT_PASSWORD_HASH" ]]; then
        while true; do
            read -sp "Entrez le mot de passe (Échap pour annuler) : " PASSWORD
            if [[ $PASSWORD == $'\e' ]]; then cancel_config; fi
            echo
            read -sp "Confirmez le mot de passe (Échap pour annuler) : " PASSWORD_CONFIRM
        ...
        read -p "Voulez-vous modifier le mot de passe ? (o/N, Échap pour annuler) : " MODIFY_PASSWORD
        if [[ $MODIFY_PASSWORD == $'\e' ]]; then cancel_config; fi
        if [[ "$MODIFY_PASSWORD" == "o" || "$MODIFY_PASSWORD" == "O" ]]; then
            while true; do
                read -sp "Entrez le nouveau mot de passe (Échap pour annuler) : " PASSWORD
                if [[ $PASSWORD == $'\e' ]]; then cancel_config; fi
                echo
                read -sp "Confirmez le nouveau mot de passe (Échap pour annuler) : " PASSWORD_CONFIRM
                if [[ $PASSWORD_CONFIRM == $'\e' ]]; then cancel_config; fi
                echo
                if [[ -z "$PASSWORD" ]]; then
                    echo "Le mot de passe ne peut pas être vide. Veuillez entrer une valeur."
...
while true; do
    ...
    # Afficher l'état du conteneur Wireguard
    ...
    if [[ -f "$DOCKER_COMPOSE_FILE" ]]; then
        ...
        echo -e "\e[1;31m⚠️  Le fichier docker-compose.yml n'est pas encore généré.\e[0m"
    fi
    ...
    if [[ -f "$DOCKER_COMPOSE_FILE" ]]; then
        case $ACTION in
            ...
            6)
                ...
                while (( ATTEMPTS < MAX_ATTEMPTS )); do
                    read -sp "Entrez le mot de passe technique pour confirmer la réinitialisation (Échap pour annuler) : " RESET_PASSWORD
                    echo
                    if [[ "$RESET_PASSWORD" == $'\e' ]]; then
                        echo -e "\e[1;33mRéinitialisation annulée par l'utilisateur.\e[0m"
                        break
                    fi
        ...
        case $ACTION in
            1)
                echo -e "\e[1;33m⚙️  Création et configuration du fichier docker-compose.yml...\e[0m\n"
                configure_values
                ;;
            2)
                echo -e "\e[1;32mAu revoir ! 👋\e[0m"
            ...
            7)
                ...
                ;;
            *)
                echo -e "\e[1;31mChoix invalide. Veuillez entrer 1, 7 ou 8.\e[0m"
                ;;
        esac
    fi
